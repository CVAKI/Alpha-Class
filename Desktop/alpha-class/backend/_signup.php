...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Validate reference code exists in database
        $refStmt = $conn->prepare("SELECT referencecode FROM reference_code WHERE referencecode = ?");
        ...
        $refStmt->bind_param("s", $reference);
        $refStmt->execute();
        $refStmt->store_result();

        if ($refStmt->num_rows === 0) {
            ob_end_clean();
